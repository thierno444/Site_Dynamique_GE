...
<div class="container">
                ...
                <tbody>
                    <?php while ($row = $search_results->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <button class="btn-modifier" onclick="window.location.href='edit_student.php?id=<?php echo $row['id']; ?>'">Modifier</button>
                                <button class="btn-archiver" onclick="window.location.href='archive_student.php?id=<?php echo $row['id']; ?>&archive=<?php echo $row['archive'] ? 0 : 1; ?>'"><?php echo $row['archive'] ? 'Désarchiver' : 'Archiver'; ?></button>
                                <button class="btn-supprimer" onclick="if(confirm('Êtes-vous sûr de vouloir supprimer cet étudiant ?')) { window.location.href='delete_student.php?id=<?php echo $row['id']; ?>'; }">Supprimer</button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
    ...
    <div class="admin-action-card">
            ...
            <p>Ajoutez un nouvel administrateur pour gérer le système.</p>
            <button onclick="window.location.href='register_admin.php'">Créer</button>
        </div>
        <div class="admin-action-card">
            <h3>Liste des administrateurs</h3>
            <p>Modifiez les informations d'un administrateur existant.</p>
            <button onclick="window.location.href='admin_management.php'">Lister</button>
        </div>
    </div>
    ...
    <div class="button-group">
        ...
        <button id="add-student" onclick="window.location.href='register_student.php'">Inscrire un Étudiant</button>
        <button id="manage-grades" onclick="window.location.href='note_etudiant.php'">Gestion des Notes de L'étudiant</button>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                while ($row = $result_active->fetch_assoc()) {
                    ...
                    <td>Actif</td>
                    <td>
                        <button class='btn-modifier' onclick=\"window.location.href='edit_student.php?id={$row['id']}'\">Modifier</button>
                        <button class='btn-archiver' onclick=\"window.location.href='archive_student.php?id={$row['id']}&archive=1'\">Archiver</button>
                        <button class='btn-supprimer' onclick=\"if(confirm('Êtes-vous sûr de vouloir supprimer cet étudiant ?')) { window.location.href='delete_student.php?id={$row['id']}'; }\">Supprimer</button>
                    </td>
                    </tr>";
                }
                ?>
        ...
        <table>
            ...
            <tbody>
                ...
                while ($row = $result_archived->fetch_assoc()) {
                    ...
                    <td>
                        <button class='btn-modifier' onclick=\"window.location.href='edit_student.php?id={$row['id']}'\">Modifier</button>
                        <button class='btn-archiver' onclick=\"window.location.href='archive_student.php?id={$row['id']}&archive=0'\">Désarchiver</button>
                        <button class='btn-supprimer' onclick=\"if(confirm('Êtes-vous sûr de vouloir supprimer cet étudiant ?')) { window.location.href='delete_student.php?id={$row['id']}'; }\">Supprimer</button>
                    </td>
                    </tr>";
                }
    ...
    <table>
        ...
        <tbody>
            ...
            while ($row = $result_total_students->fetch_assoc()) {
                ...
                <td>
                    <button class='btn-modifier' onclick=\"window.location.href='edit_student.php?id={$row['id']}'\">Modifier</button>
                    <button class='btn-archiver' onclick=\"window.location.href='archive_student.php?id={$row['id']}&archive=" . ($row['archive'] ? 0 : 1) . "'\">" . ($row['archive'] ? 'Désarchiver' : 'Archiver') . "</button>
                    <button class='btn-supprimer' onclick=\"if(confirm('Êtes-vous sûr de vouloir supprimer cet étudiant ?')) { window.location.href='delete_student.php?id={$row['id']}'; }\">Supprimer</button>
                </td>
                </tr>";
            }
